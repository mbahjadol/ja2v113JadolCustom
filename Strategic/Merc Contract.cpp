...
// JADOL -- MercDepartCauseHateAndRejectedByPlayer only work if MERCS_IRRESISTIBLE_PLAYER=TRUE on Ja2_Options.ini
void MercWantDepartCauseHateAndRejectedByPlayer(SOLDIERTYPE* pSoldier, SOLDIERTYPE* pOtherSoldier)
{
	CHAR16 sString[256];
	swprintf(sString,
		L"You have taught '%s' a lesson about responsibilities as a merc, and unbearable to OBEY your Command! Thus, has promised will not quarrel with '%s' till next 24 hour.",
		pSoldier->GetName(),
		pOtherSoldier->GetName());
	DoScreenIndependantMessageBox(sString, MSG_BOX_FLAG_OK, NULL);
	if (pSoldier->ubID != NOBODY)
	{
		// reset their hatedCount
		gMercProfiles[pSoldier->ubProfile].bLearnToHateCount += 24; // add next complain to next day
		// and the merc can not reject it, just whine about it, LOL
		INT8 whiningSound = 0;
		switch (PreRandom(2)) // 0 ~ 1
		{
		case 0:
			whiningSound = BATTLE_SOUND_LOWMORALE_OK;
			break;
		default:
			whiningSound = BATTLE_SOUND_LOWMORALE_ATTN;
			break;
		}
		TacticalCharacterDialogueWithSpecialEvent(pSoldier, 0, DIALOGUE_SPECIAL_EVENT_DO_BATTLE_SND, whiningSound, 500);
	}
}
