	...
	if (pData->currentDepth <= pData->maxReadDepth) //are we reading this element?
	{
		...
		else if (strcmp(name, "BACKGROUND") == 0 && pData->curElement == ELEMENT_LIST)
		{
			...
			(strcmp(name, "uiIndex") == 0 ||
				...
				// JADOL -- Adding new Benefit for Backgrounds
				strcmp(name, "tc_take_damage_reduction") == 0 ||
				strcmp(name, "tc_evade_hit_chance") == 0 ||
				strcmp(name, "tc_stat_damage_resist") == 0 ||
				strcmp(name, "tc_hand2hand_gore_dead_to_enemy") == 0 ||
				strcmp(name, "tc_regen_health") == 0
				// --
				))
		{
...
backgroundEndElementHandle(void* userData, const XML_Char* name)
{
	...
	if (pData->currentDepth <= pData->maxReadDepth)
	{
		...
		// JADOL -- Adding new Benefit for Backgrounds
		else if (strcmp(name, "tc_take_damage_reduction") == 0)
		{
			pData->curElement = ELEMENT;
			pData->curBackground.value[BG_TC_TAKE_DAMAGE_REDUCTION] = min(128, max(0, (INT16)atol(pData->szCharData)));
		}
		else if (strcmp(name, "tc_evade_hit_chance") == 0)
		{
			pData->curElement = ELEMENT;
			pData->curBackground.value[BG_TC_EVADE_HIT_CHANCE] = min(10000, max(0, (INT16)atol(pData->szCharData)));
		}
		else if (strcmp(name, "tc_stat_damage_resist") == 0)
		{
			pData->curElement = ELEMENT;
			pData->curBackground.uiFlags |= (UINT16)atol(pData->szCharData) ? BACKGROUND_STAT_DMG_RESIST : 0;
		}
		else if (strcmp(name, "tc_hand2hand_gore_dead_to_enemy") == 0)
		{
			pData->curElement = ELEMENT;
			pData->curBackground.uiFlags |= (UINT16)atol(pData->szCharData) ? BACKGROUND_HTH_GORE_DMG : 0;
		}
		else if (strcmp(name, "tc_regen_health") == 0)
		{
			pData->curElement = ELEMENT;
			pData->curBackground.uiFlags |= (UINT16)atol(pData->szCharData) ? BACKGROUND_INCREDIBLE_REGEN_HEALTH : 0;
		}
		// --
