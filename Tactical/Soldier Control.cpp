...
typedef struct
{
	...
	// JADOL -- renamed it with proper name from "fBadGuy" become "fNonProfileEntity"
	BOOLEAN			fNonProfileEntity;
	// --
...
BATTLESNDS_STRUCT	 gBattleSndsData[] =
{
	...
	"attn", 1, 0, 1, 0,
	"dying", 1, 1, 1, 1,
	"die", 1, 1, 1, 1,	// JADOL -- restore die , because dying & die is different
	"humm", 0, 0, 1, 1,
	"noth", 0, 0, 1, 1,
	"gotit", 0, 0, 1, 1,
	"lmok", 1, 0, 1, 2,
...
{
	"bad", // JADOL -- this tag will be categorized for truly civilians and will use JA2 vanilla battle sounds filename stock
	"kid",
	"zombie",
	"armed" // JADOL -- as for this one, it will be use for categorizing human entity which has capability to be armed, and will be used "ARMED****" battle sounds filename
};
...
SOLDIERTYPE& SOLDIERTYPE::operator=(const OLDSOLDIERTYPE_101& src)
{
	if ((void*)this != (void*)&src) {
		...
		this->ubBattleSoundTypeID = src.ubBattleSoundID;
...
void CheckForFreeupFromHit(SOLDIERTYPE* pSoldier, UINT32 uiOldAnimFlags, UINT32 uiNewAnimFlags, UINT16 usOldAniState, UINT16 usNewState)
{
	...
	if (usOldAniState != usNewState && (uiOldAnimFlags & ANIM_HITSTART) && !(uiNewAnimFlags & ANIM_HITFINISH) && !(pSoldier->flags.uiStatusFlags & SOLDIER_TURNINGFROMHIT))
	{
		...
		if (pSoldier->bTeam == gbPlayerNum && pSoldier->stats.bLife >= OKLIFE)
		{
			if (Random(10) == 0)
			{
				pSoldier->DoMercBattleSound((INT8)(BATTLE_SOUND_CURSE));
			}
		}
	}
...
void SOLDIERTYPE::EVENT_SoldierGotHit(UINT16 usWeaponIndex, INT16 sDamage, INT16 sBreathLoss, UINT16 bDirection, UINT16 sRange, UINT8 ubAttackerID, UINT8 ubSpecial, UINT8 ubHitLocation, INT16 sSubsequent, INT32 sLocationGrid)
{
	...
	//// callahan update start // ANJINGG LOE CALLAHAN BANGSAAT BIKIN ERROR AJA!!!!!!!
	...
	// JADOL -- Reinstate IC_GUN && IC_THROWING_KNIVE for vehicle like Tank is it realistic for Tank receive damage from throwing knive?
	else if (Item[usWeaponIndex].usItemClass & IC_GUN)
	{
		sBreathLoss += APBPConstants[BP_GET_HIT];
		ubReason = TAKE_DAMAGE_GUNFIRE;
	}
	else if (Item[usWeaponIndex].usItemClass & IC_THROWING_KNIFE)
	{
		sBreathLoss += APBPConstants[BP_GET_HIT];
		ubReason = TAKE_DAMAGE_THROWINGKNIVE;
	}
	// --
	...
	//  extracting attacker´s ammo type
		...
		|| AmmoTypes[MercPtrs[ubAttackerID]->inv[MercPtrs[ubAttackerID]->ubAttackingHand][0]->data.gun.ubGunAmmoType].explosionSize > 1)
	{
		INT8 bDeafValue;
	...
	if (this->stats.bLife >= CONSCIOUSNESS)
	{
		...
		else
		{
			...
			{
				this->DoMercBattleSound(BATTLE_SOUND_HIT);
			}
		}
	}
...
void SoldierGotHitGunFire(SOLDIERTYPE* pSoldier, UINT16 usWeaponIndex, INT16 sDamage, UINT16 bDirection, UINT16 sRange, UINT8 ubAttackerID, UINT8 ubSpecial, UINT8 ubHitLocation)
{
		...
		if (gAnimControl[pSoldier->usAnimState].ubEndHeight == ANIM_STAND)
		{
			if (ubSpecial == FIRE_WEAPON_HEAD_EXPLODE_SPECIAL)
			{
				if (gGameSettings.fOptions[TOPTION_BLOOD_N_GORE])
				{
					...
					{
						usNewGridNo = NewGridNo(pSoldier->sGridNo, (INT16)(DirectionInc((UINT8)pSoldier->ubDirection)));

						// CHECK OK DESTINATION!
						if (OKFallDirection(pSoldier, usNewGridNo, pSoldier->pathing.bLevel, pSoldier->ubDirection, JFK_HITDEATH))
						{
							usNewGridNo = NewGridNo(usNewGridNo, (INT16)(DirectionInc((UINT8)pSoldier->ubDirection)));

							if (OKFallDirection(pSoldier, usNewGridNo, pSoldier->pathing.bLevel, pSoldier->ubDirection, pSoldier->usAnimState))
							{
								// JADOL -- Last Check, i didn't 100% believe of ubSpecial flag receives in here,
								// so we must check if this soldier is still alive or no?!
								if (pSoldier->stats.bLife == 0) // if really die so let the person explode their head!
									fHeadHit = TRUE;
								// --
							}
						}
	...
	if (fBlownAway)
	{
		...
		if (pSoldier->ubBodyType < 4)
		{
			...
			// JADOL -- Adding team quote
			SayQuoteFromNearbyMercInSector(pSoldier->sGridNo, 30, QUOTE_HEADSHOT);
			// -
...
void SoldierGotHitExplosion(SOLDIERTYPE* pSoldier, UINT16 usWeaponIndex, INT16 sDamage, UINT16 bDirection, UINT16 sRange, UINT8 ubAttackerID, UINT8 ubSpecial, UINT8 ubHitLocation)
{
	...
	if (gGameSettings.fOptions[TOPTION_BLOOD_N_GORE])
	{
		if (Explosive[Item[usWeaponIndex].ubClassIndex].ubRadius >= 3 && pSoldier->stats.bLife == 0 && gAnimControl[pSoldier->usAnimState].ubEndHeight != ANIM_PRONE)
		{
			...
			{
				pSoldier->DoMercBattleSound(BATTLE_SOUND_HIT);
...
void SoldierGotHitPunch(SOLDIERTYPE* pSoldier, UINT16 usWeaponIndex, INT16 sDamage, UINT16 bDirection, UINT16 sRange, UINT8 ubAttackerID, UINT8 ubSpecial, UINT8 ubHitLocation)
{
	...
	// ******************************************************************************************
	// JADOL -- Hand to Hand Gore Kill Background -- <tc_hand2hand_gore_dead_to_enemy>
				...
				gMercProfiles[pAttackerSoldier->ubProfile].Type == PROFILETYPE_IMP)
			{
					...
					(gAnimControl[pSoldier->usAnimState].ubEndHeight & ANIM_CROUCH))
				{
					if (ubHitLocation == AIM_SHOT_HEAD)
					{
						...
						if (Chance(90)) // chance of 90% get exploding head and 10% is cryo death
						{
							...
							if (pSoldier->ubBodyType < 4)
							{
								pSoldier->ubDirection = pAttackerSoldier->ubDirection; // because this always near position so use same direction with attacker to die
								pSoldier->EVENT_InitNewSoldierAnim(JFK_HITDEATH, 0, FALSE);

								SpreadEffect(pSoldier->sGridNo, 6, 0, NOBODY, BLOOD_SPREAD_EFFECT, pSoldier->pathing.bLevel, -1);
						...
						{
							pSoldier->EVENT_InitNewSoldierAnim(CRYO_DEATH, 0, TRUE);
						}

					}
					else if (ubHitLocation == AIM_SHOT_TORSO)
					{
						// Only for mercs...
						if (pSoldier->ubBodyType < 4)
						{
							if (Chance(90)) // chance of 90% get thrown away body and 10% is cryo death
							{
								pSoldier->ubDirection = gOppositeDirection[pAttackerSoldier->ubDirection];
								pSoldier->ChangeToFlybackAnimation((UINT8)bDirection);
							}
							else
								pSoldier->EVENT_InitNewSoldierAnim(CRYO_DEATH_CROUCHED, 0, TRUE);

							SpreadEffect(pSoldier->sGridNo, 5, 0, NOBODY, BLOOD_SPREAD_EFFECT, pSoldier->pathing.bLevel, -1);
...
void SoldierGotHitVehicle(SOLDIERTYPE* pSoldier, UINT16 usWeaponIndex, INT16 sDamage, UINT16 bDirection, UINT16 sRange, UINT8 ubAttackerID, UINT8 ubSpecial, UINT8 ubHitLocation)
{
	...
	// JADOL -- Check Background <tc_evade_hit_chance>
	bool successfulyHit = true;
	INT16 chanceToEvade = pSoldier->GetBackgroundValue(BG_TC_EVADE_HIT_CHANCE);
	if (chanceToEvade > 0)
	{
		INT16 chanceToHit = (INT16)PreRandom(10000) + 1;
		if (chanceToHit < chanceToEvade)
			successfulyHit = false;
	}
	...
	case ANIM_STAND:
		...
		else if (IS_MERC_BODY_TYPE(pSoldier))
		{
			...
			pSoldier->EVENT_SetSoldierDesiredDirection(pSoldier->ubDirection);
			pSoldier->BeginTyingToFall(); // JADOL -- add 
			pSoldier->EVENT_InitNewSoldierAnim(FALLFORWARD_FROMHIT_STAND, 0, FALSE);
		}
		else
		{
			...
			else
			{
				...
				pSoldier->EVENT_SetSoldierDesiredDirection(pSoldier->ubDirection);
				pSoldier->BeginTyingToFall();
				pSoldier->EVENT_InitNewSoldierAnim(FALLFORWARD_FROMHIT_STAND, 0, TRUE);
			}
		}
		break;
	...
	case ANIM_CROUCH:
		...
		{
			pSoldier->BeginTyingToFall();
			pSoldier->EVENT_InitNewSoldierAnim(FALLFORWARD_FROMHIT_CROUCH, 0, TRUE);
		}
		else
		{
			if (successfulyHit)
	...
	// JADOL -- End Check Background <tc_evade_hit_chance>
...
void SOLDIERTYPE::EVENT_BeginMercTurn(BOOLEAN fFromRealTime, INT32 iRealTimeCounter)
{
	...
	// JADOL -- Managing background traits of <tc_regen_health>
	if (!(!(gTacticalStatus.uiFlags & TURNBASED) || !(gTacticalStatus.uiFlags & INCOMBAT))) // effective only when is in combat
	{
		if (this->HasBackgroundFlag(BACKGROUND_INCREDIBLE_REGEN_HEALTH))
		{
			//if (pSoldier->stats.bLife > 0) // check if this guy still alive?
			{
				INT8 deltaHealth = (this->stats.bLifeMax - this->stats.bLife);
				if (deltaHealth > 0) // maybe this guy receive damage before!?
				{
					INT8 regen = (INT8)PreRandom(deltaHealth) + 1;
					this->stats.bLife += regen; // we restoring it
					this->bBleeding -= regen;
...
UINT8 SOLDIERTYPE::SoldierTakeDamage(INT8 bHeight, INT16 sLifeDeduct, INT16 sBreathLoss, UINT8 ubReason, UINT8 ubAttacker, INT32 sSourceGrid, INT16 sSubsequent, BOOLEAN fShowDamage)
{
	...
	// JADOL -- creature or someone has background <tc_take_damage_reduction> > 0, can have reduction damage factor 
	if (CREATURE_OR_BLOODCAT(this) || (this->GetBackgroundValue(BG_TC_TAKE_DAMAGE_REDUCTION) > 0))
	{
		...
		else if (this->GetBackgroundValue(BG_TC_TAKE_DAMAGE_REDUCTION) > 0)
		{
			sReductionFactor = this->GetBackgroundValue(BG_TC_TAKE_DAMAGE_REDUCTION);
		}
	...
	// JADOL -- manage merc with background <tc_evade_hit_chance>
	bool successToEvade = false;
	if (this->GetBackgroundValue(BG_TC_EVADE_HIT_CHANCE) > 0)
	{
		// let's calculate this soldier chance
		if (PreRandom(10000) < this->GetBackgroundValue(BG_TC_EVADE_HIT_CHANCE))
			successToEvade = true; // yeaa, this person have success to evade from Death threats
	}
	if (!successToEvade)
	{
...
BOOLEAN SOLDIERTYPE::InternalDoMercBattleSound_DoSubSound(SOLDIERTYPE* pSoldier, UINT8 ubBattleSoundID)
{
	BOOLEAN				fDoSub = FALSE;
	INT32				uiSubSoundID = 0;

	// If we are a creature, etc, pick a better sound... set fDoSub TRUE/FALSE for Creature or Vehicle, and set uiSubSoundID with proper subsoundid
	if (ubBattleSoundID == BATTLE_SOUND_HIT)
	{
		switch (pSoldier->ubBodyType)
		{
		case COW:
	...
	if (ubBattleSoundID == BATTLE_SOUND_DIE)
	{
		switch (pSoldier->ubBodyType)
		{
		case COW:
	...
	{
		fDoSub = TRUE;
		if (ubBattleSoundID == BATTLE_SOUND_ATTN)
...
// JADOL -- Adding and merging BAD category of battle sound effects from Vengeage Reloaded
...
BOOLEAN SOLDIERTYPE::InternalDoMercBattleSound(UINT8 ubBattleSoundID, INT8 bSpecialCode)
{
	...
	UINT32				iFaceIndex;  
	SOLDIERTYPE* pSoldier = this;
	// fDoSub and uiSubSoundID has been removed because logic has been relocate into difference internal private function InternalDoMercBattleSound_DoSubSound()
	// BOOLEAN fDoSub = FALSE; INT32 uiSubSoundID = 0; 
	...
	// If a death sound, and we have already done ours... then returned
	if (ubBattleSoundID == BATTLE_SOUND_DIE)
	{
	...
	// JADOL -- Do Sub Sound now is relocate into small individual function
	// creature, etc, pick a better sound, it it's true then returning TRUE as success execute sub battle sound effects
	if (this->InternalDoMercBattleSound_DoSubSound(pSoldier, ubBattleSoundID))
		return(TRUE);
	// -- 
	...
	if (ubBattleSoundID == BATTLE_SOUND_OK)
	{
	...
	{
		if (ubBattleSoundID == BATTLE_SOUND_OK)
			ubBattleSoundID = BATTLE_SOUND_LOWMORALE_OK;
		else if (ubBattleSoundID == BATTLE_SOUND_ATTN)
			ubBattleSoundID = BATTLE_SOUND_LOWMORALE_ATTN;
	}

	// OK=0,COOL=1,CURSE,HIT,LAUGH,ATTN,DIE,HUMM,NOTHING,GOTIT,LMOK,LMATTN,LOCKED,ENEMY,PUNCH,KNIFE=15
	ubSoundID = ubBattleSoundID;

	// OK, build file and play!
	int entrynum = 0;
	if (pSoldier->ubProfile != NO_PROFILE)	// build file and play for person that HAVE PROFILE like NPC, RPC, PC and else
	{
	...
	else // build file and play for person that DOESN'T HAVE PROFILE OR NO_PROFILE 
	{
		// Check if we can play this!
		if (!gBattleSndsData[ubSoundID].fNonProfileEntity)
		{
			return(FALSE);
		}
		...
		{
			entrynum = 2;
			pSoldier->ubBattleSoundTypeID = 0;		// atm only one soundset for zombies
		}
		else if (pSoldier->ubBodyType == HATKIDCIV || pSoldier->ubBodyType == KIDCIV)
		{
			entrynum = 1;
			if ((ubSoundID == BATTLE_SOUND_DIE) || (ubSoundID == BATTLE_SOUND_DYING))
				ubSoundID = BATTLE_SOUND_DYING;
		}

		// JADOL -- categorize civilians to use ja2 stock battle sound effects with prefix "BAD****"
		// and use "ARMED***" for body type that can be armed with weapons
		else if (IS_HUMAN_BODY_TYPE(pSoldier))
		{
			if (IS_CIVILIAN_BODY_TYPE(pSoldier))
				entrynum = 0;
			else if (IS_ARMED_BODY_TYPE(pSoldier))
			{
				entrynum = 3;
				// because BattleSNDS/ARMED**** doesn't have type of DYING sounds so
				// we will hardcode change the ubSoundID == BATTLE_SOUND_DYING into BATTLE_SOUND_DIE
				if (ubSoundID == BATTLE_SOUND_DYING)
					ubSoundID = BATTLE_SOUND_DIE;
			}
		}
		...
		// Due to legacy reasons, the first sound can either have a '1' at the end (212_OK1.xx) or no number at all (212_HUMM.xxx)
		// Otherwise we'd have to rename quite a lot of vanilla files
		while (!BattleSoundSearchDone_Npc[entrynum][pSoldier->ubBattleSoundTypeID][ubSoundID])
		{
			// at least one sound exists (if not, we use a fallback solution anyway)
			numBattleSounds_Npc[entrynum][pSoldier->ubBattleSoundTypeID][ubSoundID] = max(1, numBattleSounds_Npc[entrynum][pSoldier->ubBattleSoundTypeID][ubSoundID]);

			UINT32 numsounds = numBattleSounds_Npc[entrynum][pSoldier->ubBattleSoundTypeID][ubSoundID];

			if (entrynum != 3) // Not ARMED
			{
				// check: is there a sound with a bigger number?			
				sprintf(zFilename, "BATTLESNDS\\%s%d_%s%d", gBattleSndsNpcHelperData[entrynum].zName, pSoldier->ubBattleSoundTypeID, gBattleSndsData[ubSoundID].zName, numsounds + 1);
			}
			else // ARMED
			{
				sprintf(zFilename, "BATTLESNDS\\ARMED\\%s\\%d\\%s%d_%s%d",
					(IS_GENDER_MALE_BODY_TYPE(pSoldier) ? "M" : "F"),
					pSoldier->ubBattleSoundTypeID,
					gBattleSndsNpcHelperData[entrynum].zName, pSoldier->ubBattleSoundTypeID, gBattleSndsData[ubSoundID].zName, numsounds + 1);
			}

			if (SoundFileExists(zFilename, zFilename_Used))
			{
				numBattleSounds_Npc[entrynum][pSoldier->ubBattleSoundTypeID][ubSoundID]++;
			}
			else
			{
				BattleSoundSearchDone_Npc[entrynum][pSoldier->ubBattleSoundTypeID][ubSoundID] = true;
			}
		}

		UINT32  maxRandom = numBattleSounds_Npc[entrynum][pSoldier->ubBattleSoundTypeID][ubSoundID];
		UINT32 soundtoplay = 1 + Random(maxRandom);

		if (entrynum != 3) // Not ARMED
			sprintf(zFilename, "BATTLESNDS\\%s%d_%s%d", gBattleSndsNpcHelperData[entrynum].zName, pSoldier->ubBattleSoundTypeID, gBattleSndsData[ubSoundID].zName, soundtoplay);
		else // ARMED
		{
			sprintf(zFilename, "BATTLESNDS\\ARMED\\%s\\%d\\%s%d_%s%d",
				(IS_GENDER_MALE_BODY_TYPE(pSoldier) ? "M" : "F"),
				pSoldier->ubBattleSoundTypeID,
				gBattleSndsNpcHelperData[entrynum].zName, pSoldier->ubBattleSoundTypeID, gBattleSndsData[ubSoundID].zName, soundtoplay);
		}
		...
		{
			if (entrynum != 3) // Not ARMED
				sprintf(zFilename, "BATTLESNDS\\%s%d_%s", gBattleSndsNpcHelperData[entrynum].zName, pSoldier->ubBattleSoundTypeID, gBattleSndsData[ubSoundID].zName);
			else // ARMED
			{
				sprintf(zFilename, "BATTLESNDS\\ARMED\\%s\\%d\\%s%d_%s",
					(IS_GENDER_MALE_BODY_TYPE(pSoldier) ? "M" : "F"),
					pSoldier->ubBattleSoundTypeID,
					gBattleSndsNpcHelperData[entrynum].zName, pSoldier->ubBattleSoundTypeID, gBattleSndsData[ubSoundID].zName);
			}
		}
	}
	...
	{
		if (ubBattleSoundID == BATTLE_SOUND_CURSE)
			spParms.uiVolume = (INT8)CalculateSpeechVolume(MIDVOLUME);
		else
...
BOOLEAN PreloadSoldierBattleSounds(SOLDIERTYPE* pSoldier, BOOLEAN fRemove)
{
	...
	for (UINT32 cnt = 0; cnt < NUM_MERC_BATTLE_SOUNDS; ++cnt)
	{
		...
		{
			if (gBattleSndsData[cnt].fPreload && gBattleSndsData[cnt].fNonProfileEntity)
			{
				if (fRemove)
				{
					SoundUnlockSample(gBattleSndsData[cnt].zName);
...
void SOLDIERTYPE::EVENT_SoldierBeginGiveItemInStealth(void)
{
	SOLDIERTYPE* pTSoldier;
...
void SOLDIERTYPE::HaultSoldierFromSighting(BOOLEAN fFromSightingEnemy)
{
	...
	if (this->pTempObject != NULL && fFromSightingEnemy)
	{
		...
		else
		{
			...
			AutoPlaceObject(this, this->pTempObject, FALSE);
			DeductPoints(this, -MinAPsToAttack(this, this->sTargetGridNo, 0, FALSE), 0); // biar ga looping terus-terusan padahal ada error di AI
		}
...
void	SOLDIERTYPE::SwitchWeapons(BOOLEAN fKnife, BOOLEAN fSideArm)
{
	...
	// JADOL -- initialize local var
	retrieveslot = (UINT8)HANDPOS;
	// --
...
BOOLEAN SOLDIERTYPE::OrderArtilleryStrike(UINT32 usSectorNr, INT32 sTargetGridNo, UINT8 bTeam)
{
	...
	else if (bTeam == OUR_TEAM)
	{
		...
		if (radiooperatorID)
		{
			...
			if (shellsfired)
				TacticalCharacterDialogueWithSpecialEvent(pSoldier, 0, DIALOGUE_SPECIAL_EVENT_DO_BATTLE_SND, BATTLE_SOUND_OK, 500);
			else
...
void	SOLDIERTYPE::DestroyEquippedRiotShield()
{
	...
	if (pObj)
	{
		...
		this->DoMercBattleSound(BATTLE_SOUND_CURSE);
	}
}
...
// JADOL -- SOLDIERTYPE::SoldierGotStatDamagedHealable(
void SOLDIERTYPE::SoldierGotStatDamagedHealable(INT8 statDamagedEnum, INT8 amountLoss, UINT16 byWhoID, INT8 whoIsCallThis)
{
	if (this->IsRiotShieldEquipped()) // don't  worry for your stat because you use shield and your shield not broken yet!
		return;

	INT8 statLoss = abs(amountLoss);

	// screen message
	std::wstring* scrMsg = NULL;

	// additional message
	const wchar_t* addMsg = L", but this guy immune from it!";

	STR16 msgOnePoint = L"%s lose %d of stat", msgMorePoint = L"%s loses %d of stat"; // initialize


	if (!this->HasBackgroundFlag(BACKGROUND_STAT_DMG_RESIST))
	{
		switch (statDamagedEnum)
		{
		case DAMAGED_STAT_HEALTH:
			if (this->stats.bLifeMax < 50)
				statLoss -= statLoss * 0.2; // give'em 20% from statloss, ok don't so cruel
			this->stats.bLifeMax -= statLoss;
			this->bBleeding -= statLoss;
			// SANDRO - added this for healing lost stats feature
			this->ubCriticalStatDamage[DAMAGED_STAT_HEALTH] += statLoss;
			break;
		case DAMAGED_STAT_DEXTERITY:
			if (this->stats.bDexterity < 50)
				statLoss -= statLoss * 0.2; // give'em 20% from statloss, ok don't so cruel
			this->stats.bDexterity -= statLoss;
			// SANDRO - added this for healing lost stats feature
			this->ubCriticalStatDamage[DAMAGED_STAT_DEXTERITY] += statLoss;
			break;
		case DAMAGED_STAT_AGILITY:
			if (this->stats.bAgility < 50)
				statLoss -= statLoss * 0.2; // give'em 20% from statloss, ok don't so cruel
			this->stats.bAgility -= statLoss;
			// SANDRO - added this for healing lost stats feature
			this->ubCriticalStatDamage[DAMAGED_STAT_AGILITY] += statLoss;
			break;
		case DAMAGED_STAT_STRENGTH:
			if (this->stats.bStrength < 50)
				statLoss -= statLoss * 0.2; // give'em 20% from statloss, ok don't so cruel
			this->stats.bStrength -= statLoss;
			// SANDRO - added this for healing lost stats feature
			this->ubCriticalStatDamage[DAMAGED_STAT_STRENGTH] += statLoss;
			break;
		case DAMAGED_STAT_WISDOM:
			if (this->stats.bWisdom < 50 && this->bTeam == OUR_TEAM)
			{
				if ((this->stats.bWisdom - statLoss) < 5) {
					// hello...
					// even an idiot still have wisdom!, if this wisdom have less than 5 then it should NOT CONSIDER a HUMAN!!
					// human with wisdom less than 5, it is impossible to walk, talk, it is literally VEGETABLE!!
					statLoss = 0;
				}
				else
					statLoss -= statLoss * 0.2; // give'em 20% from statloss, ok don't so cruel
			}
			this->stats.bWisdom -= statLoss;
			// SANDRO - added this for healing lost stats feature
			this->ubCriticalStatDamage[DAMAGED_STAT_WISDOM] += statLoss;
			break;
		case DAMAGED_STAT_LEADERSHIP:
			break;
		case DAMAGED_STAT_MARKSMANSHIP:
			break;
		case DAMAGED_STAT_MECHANICAL:
			break;
		case DAMAGED_STAT_EXPLOSIVES:
			break;
		case DAMAGED_STAT_MEDICAL:
			break;
		default:
			break;
		}
	...
	if (this->ubProfile != NO_PROFILE)
	{
		...
		{
		case DAMAGED_STAT_HEALTH:
			gMercProfiles[this->ubProfile].bLifeMax = this->stats.bLifeMax;
			msgOnePoint = New113Message[MSG113_LOSES_ONE_POINT_MAX_HEALTH];
			msgMorePoint = New113Message[MSG113_LOSES_X_POINTS_MAX_HEALTH];
			break;
		case DAMAGED_STAT_DEXTERITY:
			gMercProfiles[this->ubProfile].bDexterity = this->stats.bDexterity;
			msgOnePoint = Message[STR_LOSES_1_DEX];
			msgMorePoint = Message[STR_LOSES_DEX];
			break;
		case DAMAGED_STAT_AGILITY:
			gMercProfiles[this->ubProfile].bAgility = this->stats.bAgility;
			msgOnePoint = Message[STR_LOSES_1_AGIL];
			msgMorePoint = Message[STR_LOSES_AGIL];
			break;
		case DAMAGED_STAT_STRENGTH:
			gMercProfiles[this->ubProfile].bStrength = this->stats.bStrength;
			msgOnePoint = Message[STR_LOSES_1_STRENGTH];
			msgMorePoint = Message[STR_LOSES_STRENGTH];
			break;
		case DAMAGED_STAT_WISDOM:
			gMercProfiles[this->ubProfile].bWisdom = this->stats.bWisdom;
			msgOnePoint = Message[STR_LOSES_1_WISDOM];
			msgMorePoint = Message[STR_LOSES_WISDOM];
			break;
		case DAMAGED_STAT_LEADERSHIP:
			break;
		case DAMAGED_STAT_MARKSMANSHIP:
	...
	if (this->name[0] && this->bVisible == TRUE && !this->HasBackgroundFlag(BACKGROUND_STAT_DMG_RESIST))
	{
		switch (statDamagedEnum)
		{
		case DAMAGED_STAT_HEALTH:
			// make stat RED for a while...
			this->timeChanges.uiChangeHealthTime = GetJA2Clock();
			this->usValueGoneUp &= ~(HEALTH_INCREASE);
			break;
		case DAMAGED_STAT_DEXTERITY:
			// make stat RED for a while...
			this->timeChanges.uiChangeDexterityTime = GetJA2Clock();
			this->usValueGoneUp &= ~(DEX_INCREASE);
			break;
		case DAMAGED_STAT_AGILITY:
			// make stat RED for a while...
			this->timeChanges.uiChangeAgilityTime = GetJA2Clock();
			this->usValueGoneUp &= ~(AGIL_INCREASE);
			break;
		case DAMAGED_STAT_STRENGTH:
			// make stat RED for a while...
			this->timeChanges.uiChangeStrengthTime = GetJA2Clock();
			this->usValueGoneUp &= ~(STRENGTH_INCREASE);
			break;
		case DAMAGED_STAT_WISDOM:
			// make stat RED for a while...
			this->timeChanges.uiChangeWisdomTime = GetJA2Clock();
			this->usValueGoneUp &= ~(WIS_INCREASE);
			break;
		case DAMAGED_STAT_LEADERSHIP:
			break;
		case DAMAGED_STAT_MARKSMANSHIP:
	...
	if (this->name[0] && this->bVisible == TRUE)
	{
		if (statLoss == 1)
		{
			ScreenMsg(FONT_MCOLOR_LTYELLOW, MSG_INTERFACE, msgOnePoint, this->GetName());
		}
		else
		{
			// JADOL -- Check if target have background stat resist <tc_stat_damage_resist>
			if (!this->HasBackgroundFlag(BACKGROUND_STAT_DMG_RESIST))
				ScreenMsg(FONT_MCOLOR_LTYELLOW, MSG_INTERFACE, msgMorePoint, this->GetName(), statLoss);
			else
			{
				// debugging only
				//std::wstring* m = new std::wstring(msgMorePoint);
				//m->append(addMsg);
				//ScreenMsg(FONT_MCOLOR_LTYELLOW, MSG_INTERFACE, (STR16)m->c_str(), this->GetName(), statLoss);
				//delete m;
			}
			// --										
		}
	...
	if (whoIsCallThis != NULL) {
		OutputDebugString(String("SoldierGotStatDamagedHealable called from : %d\n", whoIsCallThis));
	}
		...
		if (pSoldier && pObj->exists())
		{
			...
			if (ItemCanBeAppliedToOthers(usItem))
			{
				...
				else
				{
					...
					this->DoMercBattleSound(BATTLE_SOUND_COOL);
				}
			}
			else
			{
	...
	if (ubPerson != NOBODY && ubPerson != this->ubID)
	{
		...
		if (pSoldier && pObj->exists() && HasItemFlag(pObj->usItem, EMPTY_BLOOD_BAG) && pSoldier->IsValidBloodDonor())
		{
			...
			if (HasItemFlag(bloodbagitem, BLOOD_BAG) || GetFirstItemWithFlag(&bloodbagitem, BLOOD_BAG))
			{
				...
				pSoldier->SoldierTakeDamage(0, BLOODDONATION_AMOUNT, 0, TAKE_DAMAGE_BLOODLOSS, this->ubID, sGridNo, 0, TRUE);
...
void SOLDIERTYPE::InitializeExtraData(void)
{
	...
	// JADOL -- initialize to manage duplicate dialogue in short while time
	this->ubLastQuoteSaidTriggered = 0;
	this->ubLastQuoteTimeTriggered = 4294967295; // because this is initialization we set that the first always need to be triggered UINT32 (4 bytes)
	// --
}
