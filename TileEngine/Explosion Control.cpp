...
#ifdef JADOLDEBUG
	ExpParams.commingFrom = commingFrom;
#endif // JADOLDEBUG
...
void GenerateExplosionFromExplosionPointer(EXPLOSIONTYPE* pExplosion)
{
	...
	if (uiFlags & EXPLOSION_FLAG_USEABSPOS)
	{
		...
		//AniParams.uiFlags		|= ANITILE_USEABSOLUTEPOS; // INI KENAPA DIMATIIN YAK?		
	}
	...
	//// JADOL -- DEBUGGING
	//if (AniParams.uiFlags & ANITILE_EXPLOSION)
	//{
	//	ScreenMsg(FONT_MCOLOR_LTYELLOW, MSG_INTERFACE, L"GEFEP  GridNo: %d, x:%d, y:%d, z:%d, tileindex:%d",
	//		sGridNo,
	//		AniParams.sX,
	//		AniParams.sY,
	//		AniParams.sZ,
	//		AniParams.usTileIndex
	//	);
	//}
	//// ---
#ifdef JADOLDEBUG
	if ((pExplosion->Params.commingFrom != NULL) && (pExplosion->Params.commingFrom > 0))
		AniParams.isIgniteExplosion = TRUE;
	else
		AniParams.isIgniteExplosion = FALSE;
#endif // JADOLDEBUG
...
BOOLEAN DamageSoldierFromBlast(UINT8 ubPerson, UINT8 ubOwner, INT32 sBombGridNo, INT16 sWoundAmt, INT16 sBreathAmt, UINT32 uiDist, UINT16 usItem, INT16 sSubsequent, BOOL fFromRemoteClient)
{
			...
			if (PreRandom(sNewWoundAmt) > gSkillTraitValues.ubDamageNeededToLoseStats)
			{
				...
				if (ubPickStat < 3) // 15% chance to lose Wisdom
				{
					...
					if (ubStatLoss > 0)
					{
						...
						// DamageSoldierFromBlast::WISDOM
						pSoldier->SoldierGotStatDamagedHealable(DAMAGED_STAT_WISDOM, ubStatLoss, MercPtrs[ubOwner]->ubID, 6);
						OutputDebugString(String("Explosion Control.cpp::DamageSoldierFromBlast::WISDOM\n"));
						// --
				...
				else if (ubPickStat < 7) // 20% chance to lose Dexterity
				{
					...
					if (ubStatLoss > 0)
					{
						// JADOL -- Check if target have background stat resist <tc_stat_damage_resist>
						// DamageSoldierFromBlast::WISDOM
						pSoldier->SoldierGotStatDamagedHealable(DAMAGED_STAT_DEXTERITY, ubStatLoss, MercPtrs[ubOwner]->ubID, 7);
						OutputDebugString(String("Explosion Control.cpp::DamageSoldierFromBlast::DEXTERITY\n"));
						// --
				...
				else if (ubPickStat < 11) // 20% chance to lose Strength
				{
					...
					if (ubStatLoss > 0)
					{
						// JADOL -- Check if target have background stat resist <tc_stat_damage_resist>
						// DamageSoldierFromBlast::WISDOM
						pSoldier->SoldierGotStatDamagedHealable(DAMAGED_STAT_STRENGTH, ubStatLoss, MercPtrs[ubOwner]->ubID, 8);
						OutputDebugString(String("Explosion Control.cpp::DamageSoldierFromBlast::STRENGTH\n"));
						// --
				...
				else if (ubPickStat < 15) // 20% chance to lose Agility
				{
					...
					{
						// JADOL -- Check if target have background stat resist <tc_stat_damage_resist>
						// DamageSoldierFromBlast::AGILITY
						pSoldier->SoldierGotStatDamagedHealable(DAMAGED_STAT_AGILITY, ubStatLoss, MercPtrs[ubOwner]->ubID, 9);
						OutputDebugString(String("Explosion Control.cpp::DamageSoldierFromBlast::AGILITY\n"));
						// --
				...
				else if (ubPickStat < 18) // 15% chance to lose Health
				{
					...
					{
						// JADOL -- Check if target have background stat resist <tc_stat_damage_resist>
						// DamageSoldierFromBlast::HEALTH
						pSoldier->SoldierGotStatDamagedHealable(DAMAGED_STAT_HEALTH, ubStatLoss, MercPtrs[ubOwner]->ubID, 10);
						OutputDebugString(String("Explosion Control.cpp::DamageSoldierFromBlast::HEALTH\n"));
						// --
	...
	// even The Chosen One still got suppression effect
	if (gGameExternalOptions.usExplosionSuppressionEffect > 0)
	{
...
BOOLEAN DishOutGasDamage(SOLDIERTYPE* pSoldier, EXPLOSIVETYPE* pExplosive, INT16 sSubsequent,
	BOOLEAN fRecompileMovementCosts, INT16 sWoundAmt, INT16 sBreathAmt, UINT8 ubOwner, BOOL fFromRemoteClient)
{
	// OJW - 20091028
	if (is_networked && is_client)
	{
	...
	if (sWoundAmt != 0 || sBreathAmt != 0)
	{
		...
		// JADOL -- Whatever, off course that gas cloud is not good for health and almost anyone suffer for it!
		// But if this Person is "The Luckiest Person in the World", let's give it a chance to evade some damage, but receive else!
		INT16 chanceToEvade = pSoldier->GetBackgroundValue(BG_TC_EVADE_HIT_CHANCE);
		if (chanceToEvade > 0)
		{
			if (PreRandom(10000) + 1 < chanceToEvade)
				sWoundAmt = 0;
		}
		// --
...
BOOLEAN ExpAffect(INT32 sBombGridNo, INT32 sGridNo, UINT32 uiDist, UINT16 usItem, UINT8 ubOwner, INT16 sSubsequent, BOOLEAN* pfMercHit, INT8 bLevel, INT32 iSmokeEffectID)
{
	...
	else
	{
		...
		InternalDropBlood(sGridNo, 0, 0, (UINT8)(__max((MAXBLOODQUANTITY - uiDist), 0)), 1);
	}
...
// JADOL - disini tempat eksekusi trigger dari remote bomb
