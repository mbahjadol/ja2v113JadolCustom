...
void HandleSoldierAI(SOLDIERTYPE* pSoldier) // FIXME - this function is named inappropriately
{
	...
	if (gfTurnBasedAI)
	{
		...
		{
			// JADOL -- Cancel and End this AI Guy Turn, because this AI entity is probably making deadlock
			if ((pSoldier->bTeam != CREATURE_TEAM) && (gTacticalStatus.ubCurrentTeam != OUR_TEAM))
			{
				...
				if ((uiTime > uiDelay || uiTime > uiShortDelay) && !gfUIInDeadlock)
				{
					LiveMessage("Breaking Deadlock");

					ScreenMsg(FONT_MCOLOR_LTRED, MSG_INTERFACE, L"#1 Aborting AI deadlock for [%d] %s %s data %d", pSoldier->ubID, pSoldier->GetName(), utf8_to_wstring(std::string(szAction[pSoldier->aiData.bAction])), pSoldier->aiData.usActionData);
					DebugAI(String("#1 Aborting AI deadlock for [%d] %s data %d", pSoldier->ubID, szAction[pSoldier->aiData.bAction], pSoldier->aiData.usActionData));
		...
		{
			// JADOL -- this player is on under ai controller for debug only
			char zzz = 1;
		}
		...
		if (pSoldier->bTeam != gTacticalStatus.ubCurrentTeam)
		{
			...
			pSoldier->flags.uiStatusFlags &= (UINT32)~SOLDIER_UNDERAICONTROL;
			return;
		}
	...
	********/
	...
	if (gfTurnBasedAI)
	{
		...
		UINT32 uiDelay = (UINT32)gGameExternalOptions.gubDeadLockDelay;
		UINT32 uiTime = (UINT32)(tCurrentTime - gtTimeSinceMercAIStart);
		// JADOL -- why still need to press escape key, thus event key is not responsive too???!!!
		// Hello the UI is in Deadlock state too!!!!
		BOOLEAN fKeyPressed = TRUE; // _KeyDown(ESC); // soo i'm ignore this thing !!!
		// --
			...
			//if ( ( GetJA2Clock() - gTacticalStatus.uiTimeSinceMercAIStart	) > ( (UINT32)gGameExternalOptions.gubDeadLockDelay * 1000 ) && !gfUIInDeadlock )
		{
			...
			ScreenMsg(FONT_MCOLOR_LTRED, MSG_INTERFACE, L"#2 Aborting AI deadlock for [%d] %s %s data %d", pSoldier->ubID, pSoldier->GetName(), utf8_to_wstring(std::string(szAction[pSoldier->aiData.bAction])), pSoldier->aiData.usActionData);
			DebugAI(String("#2 Aborting AI deadlock for [%d] %s data %d", pSoldier->ubID, szAction[pSoldier->aiData.bAction], pSoldier->aiData.usActionData));

			// JADOL -- Cancel and End this AI Guy Turn
			CancelAIAction(pSoldier, FORCE);
			EndAIGuysTurn(pSoldier);
			// --
...
{
	INT32 cnt;
	INT16 mods[4] = { (INT16) -1,(INT16) -WORLD_COLS,(INT16) 1,(INT16) WORLD_COLS };
		...
		if (!is_networked)
		{
					...
					(pSoldier->bTeam == MILITIA_TEAM && SOLDIER_CLASS_MILITIA(pSoldier->ubSoldierClass) && pSoldier->bVisible != -1)))
			{
				...
				if (!Random(5) && pSoldier->IsZombie())
				{
					...
					pSoldier->DoMercBattleSound((INT8)(BATTLE_SOUND_LAUGH));
				}
			}
		}
